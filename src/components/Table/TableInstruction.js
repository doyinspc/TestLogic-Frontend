...
function CustomTable({ ...props }) {
  ...
  const [instructionName, setInstructionName] = React.useState({});
  const [themessList, setThemessList] = React.useState([]);
  const [topicssList, setTopicssList] = React.useState([]);
  const [instructionssList, setInstructionssList] = React.useState([]);
...
const updateInstructionsList = (e) =>{
  let a = e.target.value;
  settopicName(a);
  let paths = `${API_PATH}/instruction/cat/${a}` ;
    axios.get(paths, {})
    .then(res => {
      setInstructionssList(res.data);
    })
    .catch(err => {
      console.log(err)
  })
  }
  const updateInstructionsSelect = (e) =>{
    let a = e.target.value;
    setInstructionName(a);
    setSelectedInstruction(a);
    }
  ...
  const handleClick = (num) => {
     props.editInstruction(num)
  }
  const handleDelete = (num) => {
    props.deleteInstruction(num)
  }
  const handleMove = () => {
    if(moveID && topicName && topicName > 0){
      props.moveInstruction(moveID, topicName);
    }
  }
  const handleShift = () => {
    if(moveID && topicName && topicName > 0){
      props.updateInstruction({topicID:topicName}, moveID);
    }
  }
  const handleToggle= (num, active) => {
    if(active == 1)
    {
      props.updateInstruction({active:0}, num);
    }
    else
    {
      props.updateInstruction({active:1}, num);
    }
  }

  const handleGroup= (instructionID, num) => {
      props.groupInstruction(instructionID, num);
  }

  const activeID= (num) => {
    props.activateInstruction(num)
  }
  ...
  tableData.map(alu => (
    <TableRow key={alu.id} className={classes.tableRow }>
        ...
        { props.instructionx.showActions ?
  ...
  let  inst = moveID ? props.instructionx.instructions.filter((row)=>row.id == moveID)[0]:{};
  return (
    <div className={classes.tableResponsive}>
     { open ?
      ...
      <GridItem xs={12} sm={12} md={12}>
        ...
        data = {topicssList}
        value = {topicName}
        onChange = {updateInstructionsList}
      />
      </GridItem>
      <GridItem xs={12} sm={12} md={12}>
        <CustomSelect
        ...
        record = {[ 'name', '']}
        name = 'instruction'
        data = {instructionssList}
        value = {instructionName}
        onChange = {updateInstructionsSelect}
      />
      </GridItem>
      </GridContainer>
      <CardFooter style={{ display:'flex', flexDirection:'row', justifyContent: 'space-between'}}>
                  ...
                  <Button type="button" onClick={handleShift} color="primary">{'Move Instruction & Questions'}</Button>
                  </div>
                  <div>
...
const mapStateToProps = state => ({ 
  instructionx: state.instructionReducer,
