...
function CustomTable({ ...props }) {
  ...
  const [mockssList, setMockssList] = React.useState([]);
  const [selectedMock, setSelectedMock] = React.useState([]);
...
const updateMocksList = (e) =>{
  let a = e.target.value;
  settopicName(a);
  let paths = `${API_PATH}/mock/cat/${a}` ;
    axios.get(paths, {})
    .then(res => {
      setMockssList(res.data);
    })
    .catch(err => {
      console.log(err)
  })
  }
  const updateMocksSelect = (e) =>{
    let a = e.target.value;
    setMockName(a);
    setSelectedMock(a);
    }
  ...
  const handleDelete = (num) => {
    props.deleteMock(num)
  }
  const handleMove = () => {
    if(moveID && topicName && topicName > 0){
      props.moveMock(moveID, topicName);
    }
  }
  const handleShift = () => {
    if(moveID && topicName && topicName > 0){
      props.updateMock({topicID:topicName}, moveID);
    }
  }
  const handleToggle= (num, active) => {
    if(active == 1)
    {
      props.updateMock({active:0}, num);
    }
    else
    {
      props.updateMock({active:1}, num);
    }
  }

  const handleGroup= (mockID, num) => {
      props.groupMock(mockID, num);
  }

  const activeID= (num) => {
    props.activateMock(num)
  }
  ...
  let  inst = moveID ? props.mockx.mocks.filter((row)=>row.id == moveID)[0]:{};
  return (
    <div className={classes.tableResponsive}>
     { open ?
      ...
      <CardFooter style={{ display:'flex', flexDirection:'row', justifyContent: 'space-between'}}>
                  ...
                  <Button type="button" onClick={handleShift} color="primary">{'Move Mock & Questions'}</Button>
                  </div>
                  <div>
