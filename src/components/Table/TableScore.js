...
function CustomTable({ ...props }) {
  ...
  const [scoressList, setScoressList] = React.useState([]);
...
const updateScoresList = (e) =>{
  let a = e.target.value;
  settopicName(a);
  let paths = `${API_PATH}/score/cat/${a}` ;
    axios.get(paths, {})
    .then(res => {
      setScoressList(res.data);
    })
    .catch(err => {
      console.log(err)
  })
  }
  const updateScoresSelect = (e) =>{
    let a = e.target.value;
    setScoreName(a);
    setSelectedScore(a);
    }
  ...
  const handleDelete = (num) => {
    props.deleteScore(num)
  }
  const handleMove = () => {
    if(moveID && topicName && topicName > 0){
      props.moveScore(moveID, topicName);
    }
  }
  const handleShift = () => {
    if(moveID && topicName && topicName > 0){
      props.updateScore({topicID:topicName}, moveID);
    }
  }
  const handleToggle= (num, active) => {
    if(active == 1)
    {
      props.updateScore({active:0}, num);
    }
    else
    {
      props.updateScore({active:1}, num);
    }
  }

  const handleGroup= (scoreID, num) => {
      props.groupScore(scoreID, num);
  }

  const activeID= (num) => {
    props.activateScore(num)
  }
  ...
  let  inst = moveID ? props.scorex.scores.filter((row)=>row.id == moveID)[0]:{};
  return (
    <div className={classes.tableResponsive}>
     { open ?
      ...
      <CardFooter style={{ display:'flex', flexDirection:'row', justifyContent: 'space-between'}}>
                  ...
                  <Button type="button" onClick={handleShift} color="primary">{'Move Score & Questions'}</Button>
                  </div>
                  <div>
