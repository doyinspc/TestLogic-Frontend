...
function CustomTable({ ...props }) {
  ...
  const [mockscoreName, setMockscoreName] = React.useState({});
  const [themessList, setThemessList] = React.useState([]);
  const [topicssList, setTopicssList] = React.useState([]);
  const [mockscoressList, setMockscoressList] = React.useState([]);
  const [selectedMockscore, setSelectedMockscore] = React.useState([]);
...
const updateMockscoresList = (e) =>{
  let a = e.target.value;
  settopicName(a);
  let paths = `${API_PATH}/mockscore/cat/${a}` ;
    axios.get(paths, {})
    .then(res => {
      setMockscoressList(res.data);
    })
    .catch(err => {
      console.log(err)
  })
  }
  const updateMockscoresSelect = (e) =>{
    let a = e.target.value;
    setMockscoreName(a);
    setSelectedMockscore(a);
    }
  ...
  const handleClick = (num) => {
     props.editMockscore(num)
  }
  const handleDelete = (num) => {
    props.deleteMockscore(num)
  }
  const handleMove = () => {
    if(moveID && topicName && topicName > 0){
      props.moveMockscore(moveID, topicName);
    }
  }
  const handleShift = () => {
    if(moveID && topicName && topicName > 0){
      props.updateMockscore({topicID:topicName}, moveID);
    }
  }
  const handleToggle= (num, active) => {
    if(active == 1)
    {
      props.updateMockscore({active:0}, num);
    }
    else
    {
      props.updateMockscore({active:1}, num);
    }
  }

  const handleGroup= (mockscoreID, num) => {
      props.groupMockscore(mockscoreID, num);
  }

  const activeID= (num) => {
    props.activateMockscore(num)
  }
  ...
  let  inst = moveID ? props.mockscorex.mockscores.filter((row)=>row.id == moveID)[0]:{};
  return (
    <div className={classes.tableResponsive}>
     { open ?
      ...
      <GridItem xs={12} sm={12} md={12}>
        ...
        data = {topicssList}
        value = {topicName}
        onChange = {updateMockscoresList}
      />
      </GridItem>
      <GridItem xs={12} sm={12} md={12}>
        <CustomSelect
        labelText="Mockscores"
        id="mockscore"
        record = {[ 'name', '']}
        name = 'mockscore'
        data = {mockscoressList}
        value = {mockscoreName}
        onChange = {updateMockscoresSelect}
      />
      </GridItem>
      </GridContainer>
      <CardFooter style={{ display:'flex', flexDirection:'row', justifyContent: 'space-between'}}>
                  ...
                  <Button type="button" onClick={handleShift} color="primary">{'Move Mockscore & Questions'}</Button>
                  </div>
                  <div>
...
const mapStateToProps = state => ({ 
  mockscorex: state.mockscoreReducer,
