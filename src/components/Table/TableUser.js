...
function CustomTable({ ...props }) {
  ...
  const [userssList, setUserssList] = React.useState([]);
...
const updateUsersList = (e) =>{
  let a = e.target.value;
  settopicName(a);
  let paths = `${API_PATH}/user/cat/${a}` ;
    axios.get(paths, {})
    .then(res => {
      setUserssList(res.data);
    })
    .catch(err => {
      console.log(err)
  })
  }
  const updateUsersSelect = (e) =>{
    let a = e.target.value;
    setUserName(a);
    setSelectedUser(a);
    }
  ...
  const handleDelete = (num) => {
    props.deleteUser(num)
  }
  const handleMove = () => {
    if(moveID && topicName && topicName > 0){
      props.moveUser(moveID, topicName);
    }
  }
  const handleShift = () => {
    if(moveID && topicName && topicName > 0){
      props.updateUser({topicID:topicName}, moveID);
    }
  }
  const handleToggle= (num, active) => {
    if(active == 1)
    {
      props.updateUser({active:0}, num);
    }
    else
    {
      props.updateUser({active:1}, num);
    }
  }

  const handleGroup= (userID, num) => {
      props.groupUser(userID, num);
  }

  const activeID= (num) => {
    props.activateUser(num)
  }
  ...
  let  inst = moveID ? props.userx.users.filter((row)=>row.id == moveID)[0]:{};
  return (
    <div className={classes.tableResponsive}>
     { open ?
      ...
      <CardFooter style={{ display:'flex', flexDirection:'row', justifyContent: 'space-between'}}>
                  ...
                  <Button type="button" onClick={handleShift} color="primary">{'Move User & Questions'}</Button>
                  </div>
                  <div>
