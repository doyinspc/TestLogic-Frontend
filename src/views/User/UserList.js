...
render(){
  ...
  const { classes, user, theme, themes, subjects, id } = this.props;
 
  let headArr =  user.showActions ? ["SN","User", "Action"]: ["SN","User"];
  ...
  return (
    <GridContainer>
      { user.isForm === true ?
      <GridItem xs={12} sm={12} md={12}><UserAdd /></GridItem>: null}
      <GridItem xs={12} sm={12} md={12}>
        <Card>
          <CardHeader  style={{ display:'flex', flexDirection:'row', justifyContent: 'space-between'}} color="primary">
            ...
            <h4 className={classes.cardTitleWhite}>{the.name || 'None'} Users</h4>
            
            </div>
            <div>
              <UserNavbar id={the.subjectID}/>
            </div>
          </CardHeader>
          <CardBody>
            <Table 
...
export default connect(mapStateToProps, { getUsers, getThemes })(withStyles(styles)(TableList));
